...
@dataclass
class Pi0GemmaOutputWithPast(ModelOutput):
    """
...
@dataclass
class Pi0GemmaForCausalLMOutputWithPast(ModelOutput):
    """
...
class GemmaAttention(nn.Module):
    ...
    def __init__(self, config: Pi0GemmaConfig, layer_idx: int):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_attentions:
            logger.warning_once(
                "GemmaModel is using GemmaSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` "
...
def gather_first_2d(x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    return x.reshape(bsz, seq_len, *x.shape[1:])


def scatter_first_2d(x: torch.Tensor, mask: torch.Tensor, src: torch.Tensor):
    assert x.shape[:2] == mask.shape
...
class Pi0GemmaDecoderLayer(nn.Module):
    def __init__(self, config: Pi0GemmaConfig, layer_idx: int):
        super().__init__()
...
class Pi0GemmaPreTrainedModel(PreTrainedModel):
    config_class = Pi0GemmaConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Pi0GemmaDecoderLayer"]
...
class Pi0GemmaModel(Pi0GemmaPreTrainedModel):
    """
...
class Pi0GemmaForCausalLM(Pi0GemmaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        >>> from open_pi0 import Pi0GemmaForCausalLM

        >>> model = Pi0GemmaForCausalLM.from_pretrained("google/gemma-7b")
